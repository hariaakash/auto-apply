...
export class JobApplicator {
	...
	public async initialize() {
		// Load configurations
		this.workPreferencesConfig = await loadYAMLConfig(
			WORK_PREFERENCES_YAML,
			workPreferencesSchema,
		);
		this.secretsConfig = await loadYAMLConfig(SECRETS_YAML, secretsSchema);
		this.textResumeConfig = await loadYAMLConfig(
			TEXT_RESUME_YAML,
			textResumeSchema,
		);

		// Validate resume path
		if (!existsSync(this.workPreferencesConfig.resume_path)) {
			throw new Error(
				`Resume file path doesn't exist, '${this.workPreferencesConfig.resume_path}'`,
			);
		}

		// lookahead pattern generator
		const { company_blacklist = [], title_blacklist = [] } =
	...
	protected isBlacklisted(
		...
		const titleBlacklisted = this.titleBlacklistPatterns.some((pattern) =>
			new RegExp(pattern, "i").test(jobTitle)
		);
		const companyBlacklisted = this.companyBlacklistPatterns.some((pattern) =>
			new RegExp(pattern, "i").test(company)
		);
	...
	protected filterJobs(
		jobs: IJobCard[],
	): {
		readyToProcess: IJobCard[];
		blacklisted: IBlacklistedJobCard[];
		alreadyApplied: IJobCard[];
	} {
