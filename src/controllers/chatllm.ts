import { numericQuestionTemplate, optionsTemplate, textualQuestionTemplate } from '../helpers/prompts.ts';
import { IWorkPreferences } from '../schemas/preferences.ts';
import { ITextResume } from '../schemas/textResume.ts';
import { IFormInputField, IFormOptionField, Option } from '../types/linkedin.ts';
...
export class ChatLLM extends AIAdapter {
    ...
    constructor(textResumeConfig: ITextResume, workPreferencesConfig: IWorkPreferences, modelArgs: ModelArgs) {
        super(modelArgs);
    ...
    private dedent(text: string): string {
        ...
        const indents = Array.from(text.matchAll(leadingWhitespaceRegex)).map(match => match[0]);
        const margin = indents.reduce((commonIndent, currentIndent) => {
            if (commonIndent === null) return currentIndent;
            let i = 0;
            while (i < commonIndent.length && i < currentIndent.length && commonIndent[i] === currentIndent[i]) {
                i++;
            }
    ...
    public async answerQuestionFromOptions(field: IFormOptionField): Promise<Option> {
    ...
    public async answerQuestionNumeric(field: IFormInputField, defaultExperience: number = 1): Promise<string> {
